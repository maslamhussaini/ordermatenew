-- CORRECTED RLS Policy for Financial Sessions
-- This version accounts for the actual data structure where auth.uid() doesn't directly match omtbl_users.auth_id

-- Enable RLS on financial sessions
ALTER TABLE omtbl_financial_sessions ENABLE ROW LEVEL SECURITY;

-- CORRECTED: Allow reading financial sessions for organizations the user owns
-- The user owns an organization if omtbl_organizations.auth_user_id = auth.uid()
...
-- Allow inserting financial sessions for organizations the user owns
...
-- Allow updating financial sessions for organizations the user owns
...
-- Verify the policy works by testing the query
-- Run this to test: SELECT * FROM omtbl_financial_sessions;
-- If RLS is working, you should see sessions for your organizations
