...
class SupplierWiseReportScreen extends ConsumerStatefulWidget {
  const SupplierWiseReportScreen({super.key});

  @override
  ConsumerState<SupplierWiseReportScreen> createState() =>
      _SupplierWiseReportScreenState();
}

class _SupplierWiseReportScreenState extends ConsumerState<SupplierWiseReportScreen> {
  ...
  Future<void> _loadData() async {
    ...
    try {
      ...
      final results = await repo.getSupplierWiseReport(
  ...
  Future<void> _printPdf() async {
    ...
    final columns = [
      const ReportColumn(header: 'Vendor', dataKey: 'vendor_name'),
      const ReportColumn(
          header: 'Invoices', dataKey: 'total_invoices', isNumeric: true),
      const ReportColumn(
    ...
    await Printing.layoutPdf(
      ...
      name: 'SupplierWise_${DateFormat('yyyyMMdd').format(_startDate)}.pdf',
    );
  }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        title: const Text('Supplier Wise Report',
  ...
  Widget _buildTable() {
    ...
    for (var item in _data) {
      ...
      totalInvoices += (item['total_invoices'] as int?) ?? 0;
    }
            ...
            rows: [
              ..._data.map((item) {
                return DataRow(cells: [
                  DataCell(Text(item['vendor_name']?.toString() ?? '')),
                  DataCell(Text(
                      ((item['total_invoices'] as num?)?.toInt() ?? 0).toString())),
                  DataCell(Text(
