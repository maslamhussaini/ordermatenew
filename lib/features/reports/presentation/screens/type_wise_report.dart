...
class TypeWiseReportScreen extends ConsumerStatefulWidget {
  const TypeWiseReportScreen({super.key});

  @override
  ConsumerState<TypeWiseReportScreen> createState() => _TypeWiseReportScreenState();
}

class _TypeWiseReportScreenState extends ConsumerState<TypeWiseReportScreen> {
  ...
  Future<void> _loadData() async {
    ...
    try {
      ...
      final results = await repo.getTypeWiseReport(
  ...
  Future<void> _printPdf() async {
    ...
    final columns = [
      ...
      const ReportColumn(
          header: 'Qty', dataKey: 'total_quantity', isNumeric: true),
      const ReportColumn(
    ...
    final bytes = await service.generateReport(
      ...
      totalKeys: ['total_quantity', 'total_amount'],
    );
    await Printing.layoutPdf(
      onLayout: (PdfPageFormat format) async => bytes,
      name: 'TypeWise_${DateFormat('yyyyMMdd').format(_startDate)}.pdf',
    );
  }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        title: const Text('Type Wise Report',
  ...
  Widget _buildTable() {
    ...
    for (var item in _data) {
      totalQty += (item['total_quantity'] as num?)?.toDouble() ?? 0.0;
            ...
            rows: [
              ..._data.map((item) {
                return DataRow(cells: [
                  ...
                  DataCell(Text(
                      ((item['total_quantity'] as num?)?.toDouble() ?? 0.0).toStringAsFixed(0))),
                  DataCell(Text(
