...
class GrossSalesReportScreen extends ConsumerStatefulWidget {
  const GrossSalesReportScreen({super.key});

  @override
  ConsumerState<GrossSalesReportScreen> createState() =>
      _GrossSalesReportScreenState();
}

class _GrossSalesReportScreenState extends ConsumerState<GrossSalesReportScreen> {
  ...
  Future<void> _loadData() async {
    ...
    try {
      ...
      final summary = await repo.getGrossSalesSummary(
  ...
  Future<void> _printPdf() async {
    ...
    final columns = [
      ...
      const ReportColumn(header: 'Amount', dataKey: 'gross_amount', isNumeric: true),
    ];
    ...
    final bytes = await service.generateReport(
      ...
      totalKeys: ['gross_amount'],
    );
    await Printing.layoutPdf(
      onLayout: (PdfPageFormat format) async => bytes,
      name: 'GrossSales_${DateFormat('yyyyMMdd').format(_startDate)}.pdf',
    );
  }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        title: const Text('Gross Sales Report',
  ...
  Widget _buildBody() {
    ...
    final total = (_summary['total_gross_sales'] as num?)?.toDouble() ?? 0.0;
    final count = (_summary['total_invoices'] as int?) ?? 0;
      ...
      child: Column(
        children: [
          Row(
            children: [
              Expanded(
                child: _summaryCard('Total Invoices', count.toString(),
                    Icons.receipt_long, Colors.blue),
              ),
              const SizedBox(width: 12),
              Expanded(
                child: _summaryCard(
                    'Gross Sales', '$currency ${total.toStringAsFixed(2)}',
              ...
              child: DataTable(
                ...
                rows: _details.map((item) {
                  ...
                  return DataRow(cells: [
                    ...
                    DataCell(Text(
                        '$currency ${((item['gross_amount'] as num?)?.toDouble() ?? 0.0).toStringAsFixed(2)}')),
                  ]);
                }).toList(),
              ),
            ),
